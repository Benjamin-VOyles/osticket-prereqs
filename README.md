...
1.) The first step is create a virtual machine via https://portal.azure.com/. Setup the virtual machine with Windows 10 Pro, version 22H2. Note, you will want to create a virtual machine with at the very least 2 vcpus and 16 gbs of memory for optimal functioning.

2.) Once the virtual machine has been created, connect to it with the public ip address the virtual machine is setup with. Then connect using the remote desktop connection app. 
</p>
<br />
...
3.) When the virtual machine has connected, go to the control panel and open programs. Then select Turn Windows features on and off.
...
Please make sure all Common HTTP Features are checked.
 
 To be sure that the IIS is installed / enabled use a personally preferred browser and search for 127.0.0.1 
  It should look like similar to the image provided. 
...
<p>
  ...
  Fill out the page as required except the Database Settings at the bottom of the page. That will covered further detail later on. 
...
<p>
  ...
  When the program is open make sure to create a new session in it.
...
<p>
  ...
  Once connected to the session, return to the browser to finish setting everything up. Under the Database Settings in the browser the username will be root and the password will be Password1.
  
  Now create a new database within HeidiSQL. In Heidi right click on the left side where is says "Unnamed", select "create new", and then select "database". Name the new database osTicket. Once we have the new database setup go back to the osTicket browser and under MySQL Database type in osTicket.
...
<p>
  ...
  The last step is to do some clean up. Delete the setup folder in the system. 
...
<p>
  ...
  The final step is to login to osTicket within the browser.
...
<p>
  ...
  Congratulations! osTicket has now been successfully installed!
